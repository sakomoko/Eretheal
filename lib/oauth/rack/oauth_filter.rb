    ...
    class OAuthFilter
      ...
      def call(env)
        ...
        if token_string = oauth2_token(request)
          if token = Oauth2Token.where(:invalidated_at.exists => false).excludes(authorized_at: nil).where(token: token_string).first
            ...
            if request_proxy.token
              oauth_token = client_application.tokens.where(token: request_proxy.token, :invalidated_at.exists => false).excludes(:authorized_at => nil).first
